import pandas as pd
#faseP
file1 = "/home/common/g.200.3d.24.imp/comuni/ondaP/misfit.fin.txt"
file2 = "/home/common/g.200.3st.24.imp/comuni/ondaP/misfit.fin.txt"
out = "differences_CC.txt"
...
def main():
    ...
    df = pd.read_csv(args.filepath, sep='\s+', header=0,
        names=["STA1", "CMP1", "STA2", "CMP2", "deltachi", "deltaCC", "Lat", "Lon"], usecols=["STA1", "CMP1", "STA2", "CMP2", "deltaCC", "Lat", "Lon"])
    df.columns = df.columns.str.strip()
    for col in ["deltaCC", "Lat", "Lon"]:
    ...
    for comp, comp_name in components.items():
        ...
        if args.metric == "deltaCC":
            ...
            vmin=subset[args.metric].min(); vmax=subset[args.metric].max()
            pygmt.makecpt(cmap="vik", series=[vmin,vmax], output=pathi+"metric_diff.cpt")
