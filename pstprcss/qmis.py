...
def main():
    ...
    df1 = pd.read_csv(args.filepath + "misfit.fin.txt",sep='\s+',header=None,
        names=["STA1", "CMP1", "STA2", "CMP2","misfit_norm_amp", "misfit_amp", "cross_corr","Lat", "Lon", "LAT", "LON"],
        usecols=["STA1", "CMP1", "STA2", "CMP2", "misfit_norm_amp", "Lat", "Lon"])
    df1.columns = df1.columns.str.strip()
    stazioni="/home/francesco/dati/gmt-6.5.0/ionian/stazioni.txt"
    stations = pd.read_csv(stazioni, sep='\s+', names=["Name", "Net", "Lat", "Lon", "Elev", "Extra"])
    for col in ["misfit_norm_amp", "Lat", "Lon"]:
    ...
    for comp, comp_name in components.items():
        ...
        if args.metric == "misfit_norm_amp":
            pygmt.makecpt(cmap="bam", series=[0, 1], output=pathi+"metric_misfit.cpt")
            print("Range of misfit_norm_amp:", subset[args.metric].min(), subset[args.metric].max())
            fig.plot(x=subset["Lon"],y=subset["Lat"],style="c0.5c",fill=subset[args.metric],cmap=pathi+"metric_misfit.cpt", pen="0.8p,black")
            fig.colorbar(cmap=pathi+"metric_misfit.cpt", frame=f"+l{args.cbar}", position="JBC+o8.0c/-24c+w23.7c/0.4c+v",box="+gwhite+p0.2p")
        else: 
            print("NO!")
